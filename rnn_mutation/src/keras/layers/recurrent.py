...
def reduce_precision(tensor, precision_bits=8):
    ...
    :return:
    """
    bits_count = 2**precision_bits
    return tf.round(bits_count * tensor) / bits_count
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            if gate_operator == 1:
                z = z * 0
            # update gate fuzzing
            elif gate_operator == 2:
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                z = z * fuzz
            # update gate precision reduction
            elif gate_operator == 11:
                z = reduce_precision(z, precision_bits=precision_num)
            ...
            if gate_operator == 3:
                r = r * 0
            # reset gate fuzzing
            elif gate_operator == 4:
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                r = r * fuzz
            # reset gate precision reduction
            elif gate_operator == 13:
                r = reduce_precision(r, precision_bits=precision_num)
            ...
            hh = self.activation(x_h + recurrent_h)
            # candidate gate clear
            if gate_operator == 5:
                hh = hh * 0
            # candidate gate fuzzing
            elif gate_operator == 6:
                print("wawawaw")
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                hh = hh * fuzz
            # candidate gate precision reduction
            elif gate_operator == 15:
                hh = reduce_precision(hh, precision_bits=precision_num)
...
class GRU(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        if special_initial == rnn_cell_index:
            ...
            hidden_cell = hidden_cell.reshape(1, cell_len)
            initial_cell = K.variable(value=hidden_cell)
            global_special_set()
...
class LSTMCell(Layer):
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            # print("#####")
            # dynamic input gate clear
            if gate_operator == 1:
                i = i * 0

            # dynamic and static input gate gaussian fuzzing
            elif gate_operator == 2:
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                i = i * fuzz

            # static input gate precision reduction
            elif gate_operator == 11:
                i = reduce_precision(i, precision_bits=precision_num)

            f = self.recurrent_activation(x_f + K.dot(h_tm1_f,
                                                      self.recurrent_kernel_f))
            # dynamic forget gate clear
            if gate_operator == 3:
                f = f * 0

            # dynamic and static forget gate gaussian fuzzing
            elif gate_operator == 4:
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                f = f * fuzz

            # static forget gate precision reduction
            elif gate_operator == 13:
                f = reduce_precision(f, precision_bits=precision_num)

            c_candidate = self.activation(x_c + K.dot(h_tm1_c,
                                                      self.recurrent_kernel_c))
            # dynamic memory cell candidate clear
            if gate_operator == 5:
                c_candidate = c_candidate * 0

            # dynamic and static memory cell candidate gaussian fuzzing
            elif gate_operator == 6:
                print("wawawawa")
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                c_candidate = c_candidate * fuzz

            # static memory cell candidate precision reduction
            elif gate_operator == 15:
                # c_candidate = tf.cast(c_candidate, tf.uint8)
                c_candidate = reduce_precision(c_candidate, precision_bits=precision_num)
                # c_candidate = tf.cast(c_candidate, tf.float32)
            c = f * c_tm1 + i * c_candidate

            o = self.recurrent_activation(x_o + K.dot(h_tm1_o,
                                                      self.recurrent_kernel_o))
            # dynamic output gate clear
            if gate_operator == 7:
                o = o * 0

            # dynamic and static output gate gaussian fuzzing
            elif gate_operator == 8:
                fuzz = np.random.normal(loc=0.0, scale=standard_deviation, size=None)
                fuzz = 1 + fuzz
                o = o * fuzz

            # static output gate precision reduction
            elif gate_operator == 17:
                o = reduce_precision(o, precision_bits=precision_num)
...
class LSTM(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        if special_initial == rnn_cell_index:
            hidden_state = np.array(ss.hidden_state)
            hidden_cell = np.array(ss.hidden_cell)
            state_len = len(hidden_state)
            cell_len = len(hidden_cell)
            hidden_state = hidden_state.reshape(1, state_len)
            hidden_cell = hidden_cell.reshape(1, cell_len)
            initial_state = K.variable(value=hidden_state)
            initial_cell = K.variable(value=hidden_cell)
            global_special_set()
            # print(special_initial)
            return super(LSTM, self).call(inputs,
                                          mask=mask,
                                          training=training,
                                          initial_state=[initial_state, initial_cell])
        else:
            # print(special_initial)
            global_special_set()
            # print(special_initial)
...
def global_set(op):
    ...
    :return:
    """
    global gate_operator
    gate_operator = op


def global_deviation_set(deviation):
    """
...
def global_precision_set(precision):
    """

    :param precision:
    :return:
    """
    global precision_num
    precision_num = precision


def global_special_set():
    """
    :return:
    """
    global special_initial
    special_initial += 1


def global_special_regulation(op):
    """
    :return:
    """
    global special_initial
...
def rnn_cell_index_set(index):
    """

    :param index:
    :return:
    """
    global rnn_cell_index
    rnn_cell_index = index
